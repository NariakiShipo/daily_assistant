...
### 🏠 導覽與 Home
- **第一次進入**:登入 →（只有一次）挑最多 4 個常用功能放進底部導覽列。
  登入頁留著「先不登入，用本機模式」的出口，不強迫既有使用者開帳號
- **底部導覽列**:左右各最多兩個功能，**中間固定 Home**。點選順序 = 左到右的位置，
  之後可以在側邊選單的「導覽列功能」重選
- **Home**:今日課程 / 經期狀態 / 今日行程 / 今日花費（另有明天預覽、本月花費摘要，
  預設關閉）。卡片可開關、可排序，右上「調整顯示」或側邊選單都能改
- **☰ 側邊選單**取代原本佔一格分頁的「設定」:最上面一眼看到兩人的配對與同步狀態，
  接著是全部功能（標示各自在導覽列的第幾格，沒放進去的仍從這裡進），再來是設定各段
- **設定改成總覽 + 子頁**:每列第二行直接寫目前狀態（已登入誰、連接了沒、
  提醒幾分鐘），不用點進去就知道有沒有設好；開發用的測試 Token 收進「開發者選項」
- **首次使用教學**:不是獨立輪播，而是直接在真實畫面上打光三步——中央 Home、左上 ☰、
  記帳的第一筆。可隨時略過，看過記在 `settings.tutorialSeen`，
  之後從側邊選單底部「重看使用教學」再看一次

### 📅 日曆
- 月曆檢視,新增/編輯/刪除行程
- 月曆上方只留**一列成員膠囊 + 「篩選」抽屜**(標籤、未完成、優先),
  有條件生效時按鈕上有數字徽章;搜尋收成右上圖示,點開才展開輸入框。
  「依時間 / 依人 / 未來 7 天」移到月曆下方,因為它切的是下面那份清單
- 新增行程只留**基本四欄**(標題、日期、時間、誰的),月曆點日期膠囊才展開;
  重複、提醒、優先順序・標籤、備註、Google 同步收進「更多選項」,
  每列右邊直接寫目前的值,設過的東西不會因為收合而看不見
- 兩人行程以顏色區分,可篩選「全部 / 個人」
- **搜尋**:比對標題、備註與標籤
...
### 💰 記帳
- **一屏看完當月**:月支出 / 月收入兩個數字 + 圓餅(圓心是結餘)+ 按日分組的明細
- **記一筆只要三下**:點分類格 → 在 App 內計算機(可加減)按金額 → 完成;
  設定頁可換成系統數字鍵盤。備註、日期、付款方式(現金 / 卡 / 轉帳)、收據照片
  收成一列膠囊,想填才填
- **歸屬分自己 / 伴侶 / 雙人**,每頁頂端一列切換;「雙人」怎麼算進個人統計可選
  **獨立一類**(預設,不計入任何人的上限)或**各算一半**
- **報表**:支出 / 收入 / 結餘 三個切換,配 日 / 月 / 年 與 ‹ › 換期間。
  支出與收入是圓餅 + 分類排行;結餘是三色折線,點任一點看那個月的數字
- **日曆檢視**:每格寫當日小計,點一天看明細,從這裡按 ＋ 會預填那天的日期
- **分類管理**:支出 / 收入分開,10 個預設支出分類 + 4 個收入分類。
  預設分類只能隱藏不能刪(舊帳目才不會失去分類);自訂分類可挑線條圖示、
  貼 Emoji 或**上傳圖片**——上傳後可拖曳、縮放調整圓形範圍再裁切成 256×256
- **當月花費上限**:自己與伴侶各一個(雙人不設)。只在記帳頁顯示「上限 · 剩」
  與一條單色細線,**不會變色、不會推播、不會跳任何超支警示**——這是刻意的
- **固定支出**:到期當天自動新增一筆真實帳目,可單獨修改或刪除那一筆;
  刪掉的月份不會在下次啟動時被補回來
...
src/
  ...
  services/
    ...
    expenses.ts            # 記帳的歸屬、拆帳、統計與固定支出(純計算,有單元測試)
    calculator.ts          # 記一筆的加減計算機
    navigation.ts          # 導覽列模組與 Home 卡片的設定正規化(純計算,有單元測試)
  screens/                 # 登入 / 選常用功能 / Home / 日曆 / 記帳 / 經期 / 課表 / 設定
  components/              # 共用 UI、導覽列、側邊選單、教學打光、線條圖示與圖表
```
